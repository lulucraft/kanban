...
public class TaskManager {
	...
	public static TaskHistory moveTask(String taskId, String newTaskProgressId, User taskUpdateUser) {
		...
		TaskProgress newTaskProgress = TaskProgressManager.getTaskProgress(id);
		...
		// Create taskHistory
		return TaskHistoryManager.createTaskHistory(task, oldTaskProgress, taskUpdateUser);
	}

	public static Task createTask(String taskName, long taskTypeId, User user) {
		TaskProgress taskProgress = TaskProgressManager.getFirstTaskProgress();
		if (taskProgress == null) return null;

		// Get task type from id
		TaskType taskType = TaskTypeManager.getTaskType(taskTypeId);
